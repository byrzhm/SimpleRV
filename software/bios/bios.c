...
int8_t *read_token(int8_t *b, uint32_t n, int8_t *ds) {
  ...
  while (i < n) {
    ...
    if (ch == '\x08' || ch == '\x7f') { // backspace character
      ...
      for (uint32_t j = 0; ds[j] != '\0'; j++) { // ds is delimiters
        if (ch == ds[j]) {
          b[i] = '\0';
          return b;
        }
...
int main(void) {
  ...
  for (;;) {
    ...
    if (strcmp(input, "file") == 0) {
      uint32_t address =
          ascii_hex_to_uint32(read_token(buffer, BUFFER_LEN, " \x0d"));
      uint32_t file_length =
          ascii_dec_to_uint32(read_token(buffer, BUFFER_LEN, " \x0d"));
      store(address, file_length);
    } else if (strcmp(input, "jal")) {
      uint32_t address =
          ascii_hex_to_uint32(read_token(buffer, BUFFER_LEN, " \x0d"));

      entry_t start = (entry_t)(address);
      start();
    } else if (strcmp(input, "lw")) {
      uint32_t address =
          ascii_hex_to_uint32(read_token(buffer, BUFFER_LEN, " \x0d"));
      ...
      *p = word;
      ...
      uint16_t half =
          ascii_hex_to_uint16(read_token(buffer, BUFFER_LEN, " \x0d"));
      uint32_t address =
          ascii_hex_to_uint32(read_token(buffer, BUFFER_LEN, " \x0d"));
      ...
      *p = half;
      ...
      uint8_t byte =
          ascii_hex_to_uint8(read_token(buffer, BUFFER_LEN, " \x0d"));
      uint32_t address =
          ascii_hex_to_uint32(read_token(buffer, BUFFER_LEN, " \x0d"));
